...
import { FormsPdfService, PdfQuestionAnswer } from './forms.pdf.services.js';
...
export class FormsService implements OnModuleInit, OnApplicationShutdown {
    ...
    private db!: Db;
    private formDocsBucket!: GridFSBucket;

    constructor(private readonly pdfService: FormsPdfService) {}
    ...
    async listForms(query: string, limit: number = 30) {
        try {
    ...
    async getPreviousQuestion(sessionId: string) {
        try {
            if (!ObjectId.isValid(sessionId)) {
                throw new Error("Invalid sessionId format");
            }
            ...
            const questions: any[] = formDoc.questions;

            const frontierIndex = questions.findIndex(
                (q) => q.required && (session.answers[q.id] === undefined || session.answers[q.id] === null || session.answers[q.id] === '')
            );
            ...
            await this.db.collection('form_sessions').updateOne(
    ...
    async saveAnswer(sessionId: string, questionId: string, answer: any) {
        try {
            ...
            const questionDef = formDoc.questions.find((q: any) => q.id === questionId);
            if (!questionDef) {
    ...
    async submitForm(sessionId: string) {
        try {
            ...
            const qa: PdfQuestionAnswer[] = formDoc.questions.map((q: any) => ({
                question: q.question,
            ...
            const documentId = uploadStream.id as ObjectId;
